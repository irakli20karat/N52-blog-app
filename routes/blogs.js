...
router.post('/new', requireAuth, uploadThumbnail, async function (req, res, next) {
    ...
    const renderError = (error) => {
        ...
        return res.render('new_blog', { email, error, consts: { MAX_TITLE_LENGTH, MAX_DESCRIPTION_LENGTH, MAX_CONTENT_LENGTH } });
    };

    if (!title.trim() || !description.trim() || !content.trim()) {
        return res.render('new_blog', { email, error: 'Missing required field', consts: { MAX_TITLE_LENGTH, MAX_DESCRIPTION_LENGTH, MAX_CONTENT_LENGTH } });
    }

    console.log(MAX_TITLE_LENGTH);
    if (title.length > MAX_TITLE_LENGTH) {
        return res.render('new_blog', { email, error: `Title length must be less than ${MAX_TITLE_LENGTH} characters`, consts: { MAX_TITLE_LENGTH, MAX_DESCRIPTION_LENGTH, MAX_CONTENT_LENGTH } });
    }

    if (description.length > MAX_DESCRIPTION_LENGTH) {
        return res.render('new_blog', { email, error: `Description length must be less than ${MAX_DESCRIPTION_LENGTH} characters`, consts: { MAX_TITLE_LENGTH, MAX_DESCRIPTION_LENGTH, MAX_CONTENT_LENGTH } });
    }

    if (content.length > MAX_CONTENT_LENGTH) {
        return res.render('new_blog', { email, error: `Content length must be less than ${MAX_CONTENT_LENGTH} characters`, consts: { MAX_TITLE_LENGTH, MAX_DESCRIPTION_LENGTH, MAX_CONTENT_LENGTH } });
    }
...
router.get('/:blogId', requireAuth, async function (req, res, next) {
    ...
    try {
        ...
        res.render("blog", {email, blog, blogs, userId});
    } catch (error) {
        return res.redirect('/blogs')
    }
});
...
router.post('/:blogId/comments/:commentId/:reaction(like|dislike)', requireAuth, async function (req, res, next) {
    ...
    try {
        ...
        res.status(200).send({message: 'Comment Liked/Disliked'});
    } catch (error) {
        console.log(error);
        next(createError(500));
    }
...
router.post('/:blogId/comments/:commentId/replies/:replyId/:reaction(like|dislike)', requireAuth, async function (req, res, next) {
    ...
    try {
        ...
        res.status(200).send({message: 'Reply Liked/Disliked'});
    } catch (error) {
        console.log(error);
        next(createError(500));
    }
